...
public class TinyVideoPlayer: NSObject, TinyVideoPlayerProtocol, TinyLogging {
    ...
    public func seekTo(position: Float, cancelPreviousSeeking: Bool = true, completion: ((Bool)-> Void)? = nil) {
        ...
        print("Seeking to \(timePoint)")
        player.currentItem?.seek(to: timePoint, toleranceBefore: CMTimeMake(value: 1, timescale: 1), toleranceAfter: CMTimeMake(value: 1, timescale: 1)) { completed in
